...
class _VistaInventarioState extends State<VistaInventario> {
  ...
  /// 7. Diálogo Interactiva para Crear / Modificar Artículos
  void _showProductDialog(BuildContext context, {ProductItem? item}) {
    ...
    final maxStockController = TextEditingController(
      text: isEdit ? item.maxStock.toString() : "100",
    );
    ...
    String cat = isEdit
        ? item.category
        : (_categories.isNotEmpty ? _categories.first : "Electrónica");
    String provider = isEdit
        ? item.provider
        : (_providers.isNotEmpty ? _providers.first : "Bodega");
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          DropdownButton<String>(
                            value: _providers.contains(provider)
                                ? provider
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    final maxStock =
                        int.tryParse(maxStockController.text) ?? 100;
                    ...
                    if (name.isNotEmpty && sku.isNotEmpty) {
                      setState(() {
                        if (isEdit) {
                          ...
                          // If we edited the selected product, update details controllers
                          if (_selectedProduct == item) {
                            _selectProduct(item);
                          }
                        } else {
            ...
            ElevatedButton(
              onPressed: () {
                setState(() {
                  ...
                  if (_selectedProduct == item) {
                    _selectedProduct = _products.isNotEmpty
                        ? _products.first
                        : null;
                    if (_selectedProduct != null) {
